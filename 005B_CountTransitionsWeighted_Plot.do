...
//}}}1
// [3] Count by 5 year age group and store results in matrices {{{1
// ----------------------------------------------------------------------------
...
forvalues i_meps = 0/1 {
    forvalues i_age = 1/15 {
        ...
        // Weigthed
        //tabulate  healthStatus F_healthStatusD  [iweight = `weight'] if d_MEPS==`i_meps' & lage == `i_age', row nofreq matcell(A)
        ...
        tabulate  healthStatus F_healthStatusD  [iweight = `weight'] if d_MEPS==`i_meps' & age >= `age_low' & age <= `age_hi', row nofreq matcell(A)
...
save "$DatDir/MEPS_HRS_1992-2017_TransitionCountsWeighted_Age.dta", replace
//}}}1
...
//}}}1
// [6] Plot counted relative frequencies by age group {{{1
//-----------------------------------------------------------------------------
use "$DatDir/MEPS_HRS_1992-2017_TransitionCountsWeighted_Age.dta", clear
...
foreach from_status in "Excellent" "Very Good" "Good" "Fair" "Poor" {
    ...
    graph combine fig1 fig2 fig3 fig4 fig5 fig6, ///
        ...
        name(ffig,replace)
    //
    graph export "$TexDir/MEPSvsHRS_CountWeighted_TransGraph`h_from'_Dead.svg", replace
    //
